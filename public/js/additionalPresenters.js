...
function additionalPresenter(divName) {
  if (counter == limit) {
    alert(
      "You have reached the limit of adding co-presenters. If you wish to add more, please contact the Research Days at (908) 737-3461 or researchdays@kean.edu."
    );
  } else {
    counter++;
    ...
    newdiv.innerHTML +=
      ...
      counter +
      ">" +
      "<div class='form-group row mt-4 col-md-12'><h5>Co-Presenter " +
      counter +
      "</h5></div>" +
      "<div class='row'>" +
      ...
      "<input type='number' class='form-control' id='keanID" +
      counter +
      "' name='keanID" +
      counter +
      "' min='0' max='9999999' required='true'>" +
      "</div>" +
      ...
      "<span class='input-group-text' id='keanEmail" +
      counter +
      "' name='keanEmail" +
      counter +
      "'> @kean.edu</span>" +
...
function deleteAdditionalPresenter() {
  $("#delete").attr("hidden", false);
  var lastCP = document.getElementById("DivForCP" + counter);
  lastCP.remove();
  counter--;
  if (counter <= 0) {
